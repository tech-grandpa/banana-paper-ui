...
class TestCostTracker:
    ...
    def test_openrouter_calls_use_provider_reported_cost(self):
        tracker = CostTracker()
        tracker.record_vlm_call(
            provider="openrouter",
        ...
        assert tracker.total_cost == pytest.approx(0.58)
        assert tracker.pricing_complete is True
...
class TestBudgetGuard:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("vlm", "openrouter", "anthropic/claude-opus-4.8"),
            ("image", "openrouter_imagen", "google/gemini-3-pro-image"),
        ],
    )
    def test_budget_fails_closed_when_openrouter_cost_is_missing(
        self, call_type: str, provider: str, model: str
    ) -> None:
        tracker = CostTracker(budget=1.00)

        if call_type == "vlm":
            tracker.record_vlm_call(provider, model, input_tokens=10, output_tokens=10)
        else:
            tracker.record_image_call(provider, model)

        assert tracker.pricing_complete is False
        assert tracker.is_over_budget is True

    @pytest.mark.parametrize("invalid_cost", [-0.01, float("nan"), float("inf")])
    def test_budget_rejects_invalid_provider_reported_cost(self, invalid_cost: float) -> None:
        tracker = CostTracker(budget=1.00)
        ...
        tracker.record_vlm_call(
            ...
            input_tokens=10,
            output_tokens=10,
            provider_reported_cost=invalid_cost,
        )
